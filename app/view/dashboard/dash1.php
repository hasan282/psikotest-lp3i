<?php $DataReport = $this->model()->getThisQuery('SELECT * FROM (SELECT COUNT(peserta.id) AS peserta FROM peserta) AS ps, (SELECT COUNT(id) AS sekolah FROM sekolah) AS sk, (SELECT COUNT(id) AS presenter FROM presenter WHERE is_active = 1) AS pr');
...
<div class="row">
    ...
    <div class="col-sm-4">
        <div class="small-box bg-danger">
            <div class="inner">
                <h3><?= number_format($DataCount['presenter'], 0, ",", "."); ?></h3>
                <p>Akun Pengguna</p>
            </div>
            <div class="icon">
                <i class="fas fa-chalkboard-teacher"></i>
            </div>
            <a href="<?= BaseURL('data/user'); ?>" class="small-box-footer">
                Lihat Data<i class="fas fa-arrow-circle-right ml-2"></i>
            </a>
        </div>
    </div>
        ...
        <div class="card">
            ...
            <div class="card-body table-responsive p-0">
                <table class="table table-hover">
                    ...
                    <tbody>
                        <?php $Recaps = $this->model()->getThisQuery('SELECT dates, COUNT(dates) AS jumlah FROM (SELECT SUBSTRING(id,1,6) AS dates FROM peserta) AS dt GROUP BY dates ORDER BY dates DESC LIMIT 5');
